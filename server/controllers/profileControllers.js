...
const createProfile = async (req, res) => {
  try {
   const {
    ...
    res.cookie("userId", newProfile._id);
...
const createWork = async (req, res) => {
  ...
  try {
    ...
    if (!userId) {
      return res.status(400).json({ error: "Profile not found in cookies" });
    }
    ...
    const newWork = new Work({
      title,description,cover,email,userId,sessionId
      });
...
const getWork = async (req, res) => {
  try {
    ...
    console.log('Current email from cookies:', email);
    ...
    if (req.params.id) {
      ...
      const workId = req.params.id;
      console.log('Work ID:', req.params.id);
      const work = await Work.findOne({ _id: workId, userId, email });
      ...
      // If no ID is provided, fetch all works for the userId and email
      const data = await Work.find({ email, userId }).sort({ createdAt: -1 }).exec();
      console.log("dashboard task : ", data);
